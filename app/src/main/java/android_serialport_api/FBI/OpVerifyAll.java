	...
	public void run() {
		try {
			...
			if ((fingerList == null) || (fingerList.length == 0)) {
				...
				// It is supposed that device contains only templates enrolled
				// by
				// this sample. In opposite case, verifyEx() with listed
				// templates
				// would be preferred call.
				index = mConn.verifyAll(null, null, null, null, null, null, null, PtConstants.PT_BIO_INFINITE_TIMEOUT,
						true, null, null, null);
				if (-1 != index) {
					// Display finger ID
